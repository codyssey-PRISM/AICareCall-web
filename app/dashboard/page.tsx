  ...
  useEffect(() => {
    async function initDashboard() {
      try {
        ...
        if (currentElderId) {
          const dashboardData = await dashboardApi.getDashboard(currentElderId);
        ...
        {isLoading && (
          <div className="flex items-center justify-center h-full min-h-screen">
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-slate-500 font-medium">소리가 데이터를 불러오고 있어요...</p>
            </div>
          </div>
        )}
              ...
              <div className="px-8 py-4 flex items-center justify-between w-full">
                <div className="flex items-center gap-4">
                  <div className="relative group cursor-pointer">
                    ...
                    {data && <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-emerald-500 rounded-full border-2 border-white animate-pulse"></div>}
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      {data ? (
                        <>
                          <h1 className="text-2xl font-black text-slate-900 tracking-tight">{data.elder.name}</h1>
                          <span className="text-lg font-bold text-slate-500">님의 오늘</span>
                        </>
                      ) : (
                        <>
                          <h1 className="text-2xl font-black text-slate-400 tracking-tight">등록되지 않음</h1>
                          <button
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {data?.today_highlight && (
                        <span className="px-3 py-1 rounded-lg bg-white/20 backdrop-blur-md text-xs font-bold text-white/90 border border-white/30">{data.today_highlight.call_time}</span>
                      )}
                    </div>
                    {data?.today_highlight ? (
                      <p className="text-base text-white/95 font-semibold leading-relaxed">
                        {highlightKeywords(data.today_highlight.message, data.today_highlight.tags)}
                      </p>
                    ) : (
                      <p className="text-base text-white/95 font-semibold leading-relaxed">
                        아직 오늘의 통화 기록이 없어요. 소리가 곧 따뜻한 목소리와 마음을 이어줄 거예요!
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                <div className="relative bg-violet-50/50 p-6 rounded-lg border border-violet-200/60 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:bg-gradient-to-br hover:from-violet-50 hover:to-violet-100 hover:border-violet-300 transition-all duration-300 group overflow-hidden">
                  ...
                  <div className="relative">
                    ...
                    <p className="text-3xl font-black text-violet-900 tracking-tight mb-2">{data?.weekly_stats?.call_attempts?.count || 0}회</p>
                    <p className="text-xs text-violet-600 font-medium">{data ? getAttemptsDescription(data.weekly_stats.call_attempts.count) : '아직 통화를 시도하지 않았어요'}</p>
                  </div>
                </div>
                ...
                <div className="relative bg-emerald-50/50 p-6 rounded-lg border border-emerald-200/60 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:bg-gradient-to-br hover:from-emerald-50 hover:to-emerald-100 hover:border-emerald-300 transition-all duration-300 group overflow-hidden">
                  ...
                  <div className="relative">
                    ...
                    <p className="text-sm font-bold text-emerald-700 mb-1">주간 통화 성공 횟수</p>
                    <p className="text-3xl font-black text-emerald-900 tracking-tight mb-2">{data?.weekly_stats?.call_success_count?.count || 0}회</p>
                    <p className="text-xs text-emerald-600 font-medium">{data ? getSuccessDescription(data.weekly_stats.call_success_count.count, data.weekly_stats.call_attempts.count) : '첫 통화를 기다리고 있어요'}</p>
                  </div>
                </div>
                ...
                <div className="relative bg-blue-50/50 p-6 rounded-lg border border-blue-200/60 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:bg-gradient-to-br hover:from-blue-50 hover:to-blue-100 hover:border-blue-300 transition-all duration-300 group overflow-hidden">
                  ...
                  <div className="relative">
                    ...
                    <p className="text-3xl font-black text-blue-900 tracking-tight mb-2">{data?.weekly_stats?.avg_duration?.minutes || 0}분</p>
                    <p className="text-xs text-blue-600 font-medium">{data ? getAvgDurationDescription(data.weekly_stats.avg_duration.minutes) : '통화가 완료되면 표시됩니다'}</p>
                  </div>
                </div>
                ...
                <div className="relative bg-amber-50/50 p-6 rounded-lg border border-amber-200/60 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:bg-gradient-to-br hover:from-amber-50 hover:to-amber-100 hover:border-amber-300 transition-all duration-300 group overflow-hidden">
                  ...
                  <div className="relative">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-xs font-bold text-amber-700 bg-amber-100 px-2.5 py-1 rounded-md border border-amber-300">D+{data?.elder?.service_days || 0}</span>
                    </div>
                    <p className="text-sm font-bold text-amber-700 mb-1">소리와 함께</p>
                    <p className="text-3xl font-black text-amber-900 tracking-tight mb-2">{data?.elder?.service_days || 0}일</p>
                    <p className="text-xs text-amber-600 font-medium">{data ? getDaysDescription(data.elder.service_days) : '오늘부터 시작이에요!'}</p>
                  </div>
                </div>
              </div>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          <h2 className="text-xl font-black text-slate-900 flex items-center gap-2">
                            최근 통화 기록
                            <span className="px-2 py-0.5 rounded-md bg-violet-50 text-[10px] font-bold text-violet-600 border border-violet-200">AI 분석</span>
                      ...
                      <div className="mt-6">
                        {data && data.recent_calls.length > 0 ? (
                          <TimelineCallList
                            ...
                            onCallClick={(call) => router.push(`/call-history/${call.id}`)}
                          />
                        ) : (
                          <div className="space-y-4">
                            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-center gap-3">
                              <span className="text-2xl">💡</span>
                              <p className="text-sm font-bold text-blue-900">데이터는 이런 식으로 표기됩니다!</p>
                            </div>
                      ...
                      <>
                        <div className="mx-6 mb-4 rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 p-6 text-white shadow-lg">
                          <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-sm font-bold text-white/90">{data.next_scheduled_call.day_of_week}</span>
                          </div>
                          <div className="text-center py-1">
                            <div className="text-5xl font-black mb-2 font-mono tracking-tight">{data.next_scheduled_call.time_display}</div>
                            <p className="text-base font-bold text-white/95">{data.next_scheduled_call.date_display}</p>
                          </div>
                        </div>
                  ...
                  <div className="relative bg-white rounded-lg border border-slate-200 shadow-sm p-6 overflow-hidden">
                    ...
                    <div className="relative">
                      ...
                      <div className="space-y-3">
                        {data && data.this_week_schedule.filter(item => item.scheduled_times.length > 0).slice(0, 3).length > 0 ? (
                          data.this_week_schedule
                            .filter(item => item.scheduled_times.length > 0)
                            .slice(0, 3)
                            .map((item, index) => (
                              <div key={index} className="flex items-center justify-between p-3 rounded-lg border bg-slate-50 border-slate-200 hover:bg-slate-100 transition-all">
                                <div className="flex items-center gap-3">
                                  <div className="text-left">
                                    <p className="text-xs font-bold text-slate-500">{item.day_of_week}</p>
                                    <p className="text-sm font-black text-slate-700">{item.date_display}</p>
                                  </div>
                                </div>
                                <div className="text-right">
                                  <p className="text-sm font-black text-slate-700">{item.scheduled_times[0]}</p>
                                </div>
                              </div>
                            ))
                        ) : (
